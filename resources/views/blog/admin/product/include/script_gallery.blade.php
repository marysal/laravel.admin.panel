       ...
       if($('#galleryjs').length > 0) {
                       ...
                       headers: {
                        ...
                        onSubmit: function (file, ext) {
                            if(!(ext && /^(jpg|png|jpeg|gif)$/i.test(ext))) {
                                alert("Ошибка! Разрешаем только картинки");
                                return false;
                            }
                        ...
                        onComplete: function (file, response) {
                            setTimeout(function () {
                               ...
                               $('.' + buttonMulti.data('name')).append('<img src="{{asset
                               ('/uploads/gallery/')}}/' + response.file + '" style="max-height: 150px;">');
                            }, 1000);
                        }
                   }
               )
           ...
           $(".del-items").on('click', function () {
               ...
               $.ajax({
                   ...
                   data: {id: id, src: src, token: _token},
                   type: 'POST',
                   beforeSend: function () {
